from home_task.models import JobPosting
...
class JobPostingSelector:
    ...
    coalesce_country = func.coalesce(
    ...
    def _get_percentile_cte(self, min_percentile: float, max_percentile: float):
        return select(
            ...
            percentile_cont(min_percentile).within_group(asc(JobPosting.days_to_hire))
            .label("min_p10"),
        ...
        ).group_by(JobPosting.standard_job_id, JobPosting.country_code).cte("percentile_cte")
        ...
        func.min(clean_jp.c.days_to_hire)
            .filter(
                clean_jp.c.days_to_hire.between(
                    percentile_cte.c.min_p10,
                    percentile_cte.c.max_p90
                )
            )
            .label('minimum'),
